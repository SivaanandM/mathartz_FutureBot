...
public class F4_Amaze_put_side {
			...
			if (Isboxed == false)
			{
				if (ltp > (low + (low*(x/100))))
				{
					...
					else
					{
						...
						Tradeinfo ti=new Tradeinfo(identity, "F4", "BUY", "L1", ltt, strorderid, ltp);
	...
	public void L2Flow()
	{
		...
		if (Isboxed == false)
		{
		  if(ltt.after(et))
		  {
			  ...
			  Tradeinfo ti=new Tradeinfo(identity, "F4", "SELL", "L2", ltt, strorderid, ltp);
			  CommonObjects.GlobalTradeInfo.add(ti);
			  way = "L2";
			  isend = true;
			  Isboxed = true;
			...
			if (Isboxed == false)
			{
				if (ltp < baseline)
				{
					if (c==0)
					{
						...
						Tradeinfo ti=new Tradeinfo(identity, "F4", "SELL", "L3", ltt, strorderid, ltp);
						CommonObjects.GlobalTradeInfo.add(ti);
						way = "L3";
					}
					else
		...
		try
		{
			if (Isboxed == false)
			{
				if (ltp >= (nextline + (nextline*(y/100))))
				{
					...
					{
						String strorderid =null;
						Tradeinfo ti=new Tradeinfo(identity, "F4", "SELL", "L4", ltt, strorderid, ltp);
						CommonObjects.GlobalTradeInfo.add(ti);
						way ="L4";
					}
					Isboxed = true;
		...
		try
		{
			if (Isboxed == false)
			{
				if(ltp > (baseline + (baseline*(x/100))))
				{
					...
					{
						String strorderid =null;
						Tradeinfo ti=new Tradeinfo(identity, "F4", "BUY", "L5", ltt, strorderid, ltp);
						CommonObjects.GlobalTradeInfo.add(ti);
						way="L5";
			...
			if (Isboxed == false)
			{
				if(ltp < (high - (high*(x/100))))
				{
					...
					else
					{
						...
						Tradeinfo ti=new Tradeinfo(identity, "F4", "BUY", "R1", ltt, strorderid, ltp);
						CommonObjects.GlobalTradeInfo.add(ti);
						way = "R1";
		...
		try
		{
			if (Isboxed == false)
			{
				if(ltt.after(et))
				{
					...
					Tradeinfo ti=new Tradeinfo(identity, "F4", "SELL", "R2", ltt, strorderid, ltp);
					CommonObjects.GlobalTradeInfo.add(ti);
					way = "R2";
					isend = true;
			...
			if (Isboxed == false)
			{
				if (ltp > baseline)
				{
					if (c==0)
					{
						String strorderid =null;
						strorderid = placeorder(identity, qty,"SELL");
						Tradeinfo ti=new Tradeinfo(identity, "F4", "SELL", "R3", ltt, strorderid, ltp);
						way = "R3";
						CommonObjects.GlobalTradeInfo.add(ti);
						points = points-2;
					}
			...
			if (Isboxed == false)
			{
				if (ltp <= (nextline - (nextline*(y/100))))
				{
					...
					if(c==1)
					{
						String strorderid;
						strorderid = placeorder(identity, (int) (qty/2),"SELL");
						Tradeinfo ti=new Tradeinfo(identity, "F4", "SELL", "R4", ltt, strorderid, ltp);
						way = "R4";
						CommonObjects.GlobalTradeInfo.add(ti);
						points = points+1;
					}
		...
		try
		{
			if (Isboxed == false)
			{
				if(ltp < (baseline - (baseline*(x/100))))
				{
					...
					{
						strorderid = placeorder(identity, qty,"BUY");
						Tradeinfo ti=new Tradeinfo(identity, "F4", "BUY", "R5", ltt, strorderid, ltp);
						CommonObjects.GlobalTradeInfo.add(ti);
						way = "R5";
						nextline = (baseline - (baseline*(x/100)));
					}
