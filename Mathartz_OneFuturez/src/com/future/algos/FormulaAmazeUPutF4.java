...
public class FormulaAmazeUPutF4 {
	...
	public void UpdateVariables()
	{
		try
		{
			fvalue.SetAmazeCentervalues(c, lc, r, points, low, high, nextline, baseline, way,tpl,spoint,z);
			finput.setIsend(isend);
			bv = listcom.getBeastViewListByID(identity,CommonObjects.GlobalBeastViewList);
			bv.setF4Point(points);
			if ((way.equalsIgnoreCase("R4"))||(way.equalsIgnoreCase("L3"))||(way.equalsIgnoreCase("L2"))||(way.equalsIgnoreCase("R3"))||(way.equalsIgnoreCase("R2")))
			{
				if ((way.equalsIgnoreCase("R4")) && (isend))
				{
					bv.setF4PL(tpl);
				}
				else
	...
	public void AssignvVariables()
	{
		try
		{
			...
			fvalue = listcom.getAmazeCenterValuesByID(identity, CommonObjects.GlobalAmazeCenterValuesF4);
			c = fvalue.getc();
			lc = fvalue.getlc();
			r = fvalue.getr();
		...
		try
		{
			if (Isboxed == false)
			{
				if (ltp >= (nextline + (nextline*(z/100))))
				{
					z = x;
					//baseline = nextline;
					nextline = (nextline + (nextline*(x/100)));
					points=points+1;
					c=c+1;
					if(c == 1)
					{
			...
			if (Isboxed == false)
			{
				if(ltp < (spoint - (spoint*(x/100))))
				{
					...
					else
					{
						...
						String strorderid =null;
						strorderid = placeorder(identity, qty,"BUY");
						Tradeinfo ti=new Tradeinfo(identity, fname, "BUY", "R1", ltt, strorderid, ltp);
						CommonObjects.GlobalTradeInfo.add(ti);
						way = "R1";
		...
		try
		{
			if (Isboxed == false)
			{
				if(ltt.after(et))
				{
					...
					Tradeinfo ti=new Tradeinfo(identity, fname, "SELL", "R2", ltt, strorderid, ltp);
					CommonObjects.GlobalTradeInfo.add(ti);
					way = "R2";
					isend = true;
			...
			if (Isboxed == false)
			{
				if (ltp > baseline)
				{
					if (c==0)
					{
						String strorderid =null;
						strorderid = placeorder(identity, qty,"SELL");
						Tradeinfo ti=new Tradeinfo(identity, fname, "SELL", "R3", ltt, strorderid, ltp);
						way = "R3";
						CommonObjects.GlobalTradeInfo.add(ti);
						points = points-2;
						tpl = (tpl + ((ltp*(qty*75))) * -1);
			...
			if (Isboxed == false)
			{
				if (ltp <= (nextline - (nextline*(z/100))))
				{
					...
					if(c==1)
					{
						String strorderid;
						strorderid = placeorder(identity, (int) (qty/2),"SELL");
						Tradeinfo ti=new Tradeinfo(identity, fname, "SELL", "R4", ltt, strorderid, ltp);
						way = "R4";
						CommonObjects.GlobalTradeInfo.add(ti);
						points = points+1;
						tpl = (tpl + ((ltp * (int) ((qty*75)/2)) * -1));
