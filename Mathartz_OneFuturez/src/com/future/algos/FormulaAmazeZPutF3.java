...
public class FormulaAmazeZPutF3 {
		...
		try
		{
			...
			for (String id : ids) 
			{
				...
				if (finput != null)
				{
					if (!finput.getIsend())
					{
						System.out.println("SYMBOL - "+id+" LTP - "+ LTP + " LTT - "+ LTT);
						AssignvVariables();
						Amaze();
						UpdateVariables();
					}
	...
	public void AssignvVariables()
	{
		try
		{
			...
			fvalue = listcom.getAmazeValuesZByID(identity, CommonObjects.GlobalAmazeValuesF3);
			c = fvalue.getc();
			lc = fvalue.getlc();
			r = fvalue.getr();
